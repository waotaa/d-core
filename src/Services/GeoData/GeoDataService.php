...
abstract class GeoDataService
{
    ...
    {
        $configDisk = config('filesystems.geo');
        if (is_null($configDisk)) {
            throw new Exception('filesystems.geo is not set in config. Create the config or publish the filesystems config from Dennis-core');
        }
        return $configDisk;
    }
}
